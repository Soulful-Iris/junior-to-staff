...
hundred requests miss at once, and each one independently runs the 400ms query
to repopulate the cache. The database gets two hundred copies of its most
expensive query. Its hundred connection slots — the PostgreSQL default, which
nobody chose — fill instantly. Now sign-in fails too, because sign-in needs a
connection and there are none.
...
competes for the same pool. The first purchase under load is rarely hardware;
it is a pooler (PgBouncer, for PostgreSQL) and a decision about pool-full.
*Query shape*: [the junior tier](../../01-junior/05-data-and-databases/) taught
that a scan grows with the table; concurrency is worse, because two hundred
copies of one slow query fight for the same pages, locks and pool slots. *Read
pressure*: most products read far more than they write, so reads saturate
first. *Failover*: P1 taught that a dependency being down is normal; at scale
the database is the dependency, and the seconds where nobody is primary are
designed and rehearsed, not caught.

**The ladder, and what each rung bills.**

![The escalation ladder: index, then cache, then replicas, then shard last. Each rung buys headroom and bills a new failure class — write cost, staleness and stampedes, lag and failover, routing and rebalancing.](../../../assets/diagrams/scaling-order.svg)

Climb on a measured number; each rung is cheaper and more reversible than the
one above.
...
2. **Cache** — buys read headroom from a copy, and bills two failure classes.
   *Invalidation*: the copy can disagree with the truth, and enumerating every
   ...
   expires, every concurrent reader misses at once and recomputes. Protection
   is standard; name yours: a **single-flight lock** (one request recomputes,
   the rest wait or take stale), **stale-while-revalidate** (serve the expired
   value, refresh in the background — in HTTP itself since RFC 5861, 2010;
   checked 2026-09-21), **TTL jitter** (randomise expiry so keys born together
   do not expire together), **probabilistic early refresh** (each hit may
   refresh early, with odds rising near expiry). A cache with none of these is
...
3. **Replicas** — buy read scale and a standby. The bill is **staleness**:
   ...
   always a little behind, and under load "a little" has no bound. Failover is
   itself an event that can fail: promote an async replica, and writes the old
   primary acknowledged but never shipped are gone.
4. **Shard** — splits data across databases by a key; the only rung that buys
   **write** headroom. The bill is the largest: every query must know where to
   look (routing), growth means moving live data (rebalancing), and cross-shard
   transactions and joins stop being the database's job. PostgreSQL has no
   native sharding — an extension or your problem. Shard last: it is the one
   rung with no ladder down.

**Queues.** A queue buys two things: it absorbs bursts — arrivals may exceed
...
because a consumer that crashes after the work but before the acknowledgement
gets the message again; and the backlog is invisible until you instrument it —
the product looks healthy while work quietly ages.
...
2026-09-21) — can make read-process-write exactly-once while everything stays
inside Kafka; the moment your consumer writes to PostgreSQL or sends an email,
the crash-retry gap reopens. The practice is at-least-once plus **idempotent
consumers** — give every message a key, put a unique constraint where the
effect lands, let the database refuse the second delivery — or the **outbox
pattern** for the producing side: business row and outgoing message written in
one transaction, a relay publishing from the outbox table, so "saved but never
announced" cannot happen.

**Backpressure.** A queue with no bound is a memory leak with a scheduler: when
arrivals outrun service for long enough, it eats memory or disk and fails at
the worst moment, having hidden the problem until then. Every buffer needs a
...
converges. The user who pressed save lives in the meantime: the write went to
the primary, the next read hit a replica that has not seen it, and the edit is
gone from the screen. So they save again — now there are two — or they stop
...
their session. It is cheap; the user's trust is not.
...
- Round-number TTLs expiring together, and a deploy that clears every cache
  while the database finds out.
...
resource saturates first — connection pool, one query's plan, locks,
CPU, cache misses — and for each, the exact query or command and what
a bad number looks like.
...
instead of the impressive answer.

*What you should get back:* that sign-in failing beside one slow page points at
a shared resource — almost always connections — not at the page's own query. If
the first suggestion is "add Redis" with no plan read, the measuring was
skipped.
...
1. The invalidation story: every write path that must touch this key,
   enumerated. If you cannot enumerate them all, say so and fall back
   to a TTL with a stated staleness cost.
...
performance fix quietly turning into a single point of failure.
...
Demanding the dedup live in a database constraint closes the version that most
often survives review.

*What you should get back:* a consumer whose second delivery dies on a unique
constraint and moves on, an explicit bound, and both tests. "Exactly-once" as a
client-library setting is the boundary error this section exists to remove.
...
2. **Kill the consumer for ten minutes** under write load. Adding items must
   ...
   test, and after restart you can say how long the drain took. If you learned
...
4. **Write, then immediately read through the replica path**, in a loop, under
   ...
   "We have never seen it" usually means "we have never measured it."
5. **Fill the pool on purpose** — hold idle transactions open until it is
   exhausted — and learn which features share a fate with the busiest page
   before a newsletter teaches you.
...
- A load harness — any tool that can hold N concurrent requests — and a written
  baseline: p50 and p99 for the group page at rest and at N, before any change.
...
- **cache stampede** — every reader missing at once when a hot key expires and
  recomputing in parallel; also called thundering herd.
