...
A reading-list team wants a change pushed to main to become available on its website automatically. A build turns that source revision into the files or container that will run. Deployment installs that exact result in the serving environment.

If a change goes wrong, the team needs to restore a known earlier version. Replacing the code alone may not work if the newer version has already written data the older version cannot understand.

### Example walkthrough

| Action | Expected behavior |
|---|---|
| Commit A reaches main | Build and deploy an artifact identified as A. |
| Commit B deploys but its feature is problematic | Turn off the feature or restore a known compatible artifact. |
| B wrote a new data format | Check whether A can still read it before relying on rollback. |

An artifact is the built output you deploy. Keep source revision, artifact identity, feature exposure and data compatibility explicit so restoring service is a concrete operation.

## Your assignment

**Deliver:** Build automatic deployment from main and demonstrate restoring a known earlier artifact. Include the data compatibility needed for that rollback to work.
