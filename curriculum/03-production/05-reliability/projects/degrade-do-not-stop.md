...
**First slice:** Keep saved URLs local while a title provider takes 8 seconds and the interactive budget is 500 ms. **Show:** the list response within budget with a `pending` or `stale title` label, and bound/defer the fetch. Seed a cache entry from another account: it must never become a fallback, even if the provider is down.
