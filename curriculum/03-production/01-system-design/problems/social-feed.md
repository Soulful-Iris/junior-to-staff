...
|---|---|
| An ordinary creator publishes a post | Make its ID available to followers. |
| A celebrity publishes to ten million followers | Use a strategy that does not require the same immediate work as a small account. |
| A candidate post becomes private | Check current access before displaying its content. |
