...
|---|---|
| Publisher A returns a new article | Store it and make it eligible for the reader's feed. |
| Another feed includes the same article | Recognize the duplicate rather than displaying another copy. |
| Publisher A has failed for an hour | Show the last successful check time instead of claiming the feed is current. |
