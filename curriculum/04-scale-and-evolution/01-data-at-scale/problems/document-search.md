...
Employees search for words in company documents. A search index stores a processed copy of document text so queries do not need to scan every original file. Results may include a title and a short excerpt called a snippet.

The index can lag behind edits and permission changes. If access is revoked after indexing, even showing a sensitive title or snippet can leak information. The current document permissions must still control the response.

### Example walkthrough

| Action | Expected behavior |
|---|---|
| Ana searches for a phrase in an allowed document | Return its title and an allowed snippet. |
| Her access is revoked while an old search entry remains | Omit the protected result. |
| A document is deleted | Remove its index entry and prevent stale copies from being returned. |

An index is derived data, not the authority for access. Your search response must account for the gap between updating the original record and updating its search copy.

## Your assignment

**Deliver:** Build document indexing and search. Recheck current access before returning a result's title, snippet or cached content, including when the index is outdated.
