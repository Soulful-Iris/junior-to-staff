...
A bookmark application already has automated checks that report success. Its maintainers want to know whether those checks would actually notice a serious mistake, such as letting Bob edit Alice's bookmark. A green result alone does not answer that question.

In a disposable copy, deliberately introduce one small defect at a time and observe which existing checks detect it. This is an exercise in evaluating the evidence you already have. The result is a report about detected and missed behavior changes.

### Example walkthrough

| Action | Expected behavior |
|---|---|
| Remove the owner condition from bookmark editing | Observe whether an existing check reports the unauthorized edit. |
| Change the expiry comparison at its exact boundary | Observe whether the existing checks notice the changed result. |
| Restore the original behavior | Record which checks detected each defect and which gaps remain. |

A mutation is a deliberate small code change used to challenge the checks. Sensitivity describes whether the available evidence responds when the protected behavior breaks.

## Your assignment

**Deliver:** Produce a report showing which deliberate defects the existing checks detect and which they miss. Do the experiment in a disposable copy and keep each observation tied to the change that caused it.
