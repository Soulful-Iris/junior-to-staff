...
Recorded **2026-09-22** for `feature/visual-learning-interview-paths`. The two routes now contain **42 individual coding problems**, **45 project briefs** (25 section, 15 act, five spine), **21 substantive tier chapters**, practical implementation/review/concurrency labs, a runnable local browser/API/SQLite slice, separate architecture exercises, five production cases and dated research. Each route distinguishes supplied reference code from build briefs and independent assessment.

The [audit acceptance tracker](../../docs/AUDIT-IMPLEMENTATION.md) links the exact F1–F14 corrections and limits. The evidence below verifies reference artifacts; no actual learner performance or hiring probability is inferred. The [independent implementation review](../../docs/reviews/2026-09-22-implementation-review.md) records reproduced defects, fixes, and focused acceptance evidence.
...
```bash
tsc -p paths/interviews/full-stack/bookmark-editor/tsconfig.json
```
...
The final local-link check resolved **1,460 relative links**; the learning checker passed **219 Markdown files, 76 mechanism SVGs and lab invariants**. Local-link, source/asset and provenance-structure checks are distinct from source truth, rendered appearance and runtime tests. The claim checker validates 13 retained claim IDs and required provenance structure; exact source pages were inspected separately. Unsupported original research claims are explicitly withdrawn in the [ledger](../../docs/research/claim-ledger.md).
...
- The 21 original tier chapters retain their illustrations and now explain a concrete initial problem, worked values, a method and changed requirements. **All 97 original SVG byte hashes are unchanged.**
...
Interview evidence uses the **2025-09-22 lower boundary**, preferably **2026-03-22 or later**, and separates publication from event dates. Undated live official technical guidance has publication age unknown and is not counted as a recent interview report. The [research ledger](research/README.md) preserves company/level/geographic gaps; no statistical “most asked” ranking or universal senior/lead threshold is claimed.

[Interview home](README.md) · [Audit acceptance](../../docs/AUDIT-IMPLEMENTATION.md)
