...
Prerequisites: [keys, constraints, and row ownership](../../../../../tiers/01-junior/05-data-and-databases/README.md),
...
/tmp/interview-pg-venv/bin/python paths/interviews/architecture/labs/postgresql/run_schedules.py
psql "$LAB_PG_DSN" -X -v ON_ERROR_STOP=1 -f paths/interviews/architecture/labs/postgresql/plans.sql > /tmp/interview-pg-plans.txt
```
...
```bash
docker exec -i interview-pg psql -U postgres -d interview_lab -X -v ON_ERROR_STOP=1 < paths/interviews/architecture/labs/postgresql/plans.sql > /tmp/interview-pg-plans.txt
```
...
```bash
python -m unittest discover -s paths/interviews/architecture/labs/postgresql -p 'test_*.py' -v
```
